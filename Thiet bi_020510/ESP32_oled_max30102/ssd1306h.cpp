...
// void SSD1306::drawChar(int x, int y, unsigned char c, int color)
// {
//   uint16_t data;
...
//   }
// }
void SSD1306::drawChar(int x, int y, unsigned char c, int color, int BIG) {
    uint16_t data;
    for (uint8_t i = 0; i < FONT_WIDTH; i++) {
        data = pgm_read_byte_near(&font_bitmap[c - FONT_START ][i]);
        if (BIG == 2) data = Stretch(data);
...
void SSD1306::drawStr(int x, int y, const char *s, int BIG) {
    if (!inPage(y, FONT_HEIGHT * BIG)) return;
    for (int k = 0; s[k] != '\0'; ++k) {
        drawChar(x, y, s[k], 1, BIG);
        x += (FONT_WIDTH + 1) * BIG;
    }
}

void SSD1306::drawStr(int x, int y, const __FlashStringHelper *s, int BIG) {
    if (!inPage(y, FONT_HEIGHT * BIG)) return;
    PGM_P p = reinterpret_cast<PGM_P>(s);
    while (1) {
        unsigned char c = pgm_read_byte(p++);
        if (c == 0) break;
        drawChar(x, y, c, 1, BIG);
        x += (FONT_WIDTH + 1) * BIG;
    }
}
